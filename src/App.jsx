...
function Pipeline({ hs }) {
  ...
  const runScan = useCallback(async (input) => {
    ...
    try {
      log("SCAN", "Indeed", "Searching call center & phone agent openings...");
      await sleep(400);
      log("SCAN", "LinkedIn Jobs", "Scanning financial services postings...");
      await sleep(300);
      log("SCAN", "ZipRecruiter", "Querying phone representative roles...");
      await sleep(300);
      log("SCAN", "Glassdoor", "Cross-referencing active listings...");
      await sleep(200);
      log("SCAN", "Google Jobs", "Aggregating results across boards...");
      ...
      const companies = useSignals.map(s => {
        ...
        // 2. COMPANY SIZE (weight 15%) — assume mid-market since we filtered
        const sizeScore = 2;
        ...
        // 4. AI VOICE READINESS (weight 20%) — default optimistic, no data
        const aiScore = 2;
    ...
    try {
      ...
      for (const co of picked) {
        try {
          ...
          log("DM", "Apollo.io", `Searching contacts at ${co.name}...`);
          await sleep(400);
          log("DM", "LinkedIn", `Verifying title, tenure, current role...`);
          await sleep(200);
          log("DM", "Hunter.io", `Resolving email pattern...`);
          await sleep(200);
          ...
          log("OK", "Apollo.io", `${dm.name} — ${dm.title} (${dm.confidence} confidence)`, "success");
          if (dm.background) log("BG", "Background", dm.background);
          if (dm.email_guess) log("EM", "Hunter.io", `Verified: ${dm.email_guess}`);
  ...
  const runOutreach = useCallback(async () => {
    ...
    try {
      ...
      for (let idx = 0; idx < picked.length; idx++) {
        ...
        try {
          log("ROI", "ROI Engine", `Modeling costs for ${item.company.name}...`);
          await sleep(300);
          log("ROI", "BLS Data", `Pulling avg salary for ${item.signal?.location || "US"}...`);
          await sleep(200);
          log("COPY", "Copywriter", `Personalizing outreach for ${item.dm.name}...`);

          const bgContext = item.dm.background ? `\n\nDM BACKGROUND (use to personalize): ${item.dm.background}` : "";
          const s4 = await callClaude(
            "B2B sales copywriter at an AI voice startup. Write punchy, specific, human outreach. Return ONLY valid JSON.",
            `ROI+OUTREACH for ${item.company.name} (${item.company.employees} emp, ${item.company.revenue || "unknown"} rev, ${item.company.industry || item.signal?.industry || "financial services"}). Currently hiring ${item.signal?.num_openings || 8} phone agents at ${item.signal?.location || "US"}. Feather AI voice platform=$0.07/min.${bgContext}\n\nDM: ${item.dm.name}, ${item.dm.title}\n\n1. ROI CALC: Current cost = (${item.signal?.num_openings || 8} agents x avg salary $45K x 1.3 benefits + $4K training each). Feather cost = (50 calls/agent/day x 5min avg x 250 days x $0.07/min). Show clear numbers.\n\n2. COLD EMAIL (ready to send, <100 words): Reference their specific job posting for ${item.signal?.role_title || "phone agents"}. Lead with the ROI number. Mention Feather by name. End with a specific ask (15-min call this week). Subject line <50 chars. Sign as "Krish" from Feather.\n\n3. LINKEDIN CONNECTION NOTE (<300 chars): Short, personal, reference something from their background if available. No pitch - just connect.\n\n4. LINKEDIN FOLLOW-UP MESSAGE (<150 words): After they accept. Reference the hiring signal. Share the ROI number. Ask for 15 mins.\n\n5. LINKEDIN THOUGHT LEADERSHIP POST (<200 words): Provocative take on AI replacing call centers in their industry. End with a question.\n\nReturn JSON:\n{"roi":{"hiring_annual":0,"feather_annual":0,"savings":0,"pct":0},"email":{"subject":"","body":""},"linkedin":{"note":"","followup":""},"post":""}`, false);
          ...
          {phase === "gate1" && (
            <div className="fu" style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                }}>Find decision makers ({approved1.size}) &rarr;</button>
              </div>
                    ...
                    <div key={i} onClick={() => { const n = new Set(approved1); on ? n.delete(c.name) : n.add(c.name); setApproved1(n); }}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: "#9ca3af", marginLeft: 28 }}>{c.employees} employees &middot; {c.revenue || "unknown rev"} &middot; {c.reasoning}</div>
                          {sig && <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 28, marginTop: 4 }}>
                            ...
                            <span style={{ fontSize: 10, color: "#9ca3af" }}>{sig.num_openings || "?"}x {sig.role_title} &middot; {sig.location || "US"}</span>
                      ...
                      {c.scores && <div style={{ marginLeft: 28, background: "#f9fafb", borderRadius: 8, padding: "10px 12px" }} onClick={e => e.stopPropagation()}>
                        <div style={{ fontSize: 9, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 8 }}>ICP scorecard &mdash; weighted {c.total_score}/10</div>
            ...
            <div className="fu" style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <button onClick={runOutreach} disabled={approved2.size === 0} style={{
                  ...
                  border: "none", borderRadius: 8, padding: "8px 20px", fontSize: 13, fontWeight: 600,
                }}>Generate outreach ({approved2.size}) &rarr;</button>
              </div>
              <div style={{ display: "grid", gap: 8 }}>
                {enriched.map((e, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => { const n = new Set(approved2); on ? n.delete(e.company.name) : n.add(e.company.name); setApproved2(n); }}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: 11, color: "#9ca3af" }}>{e.company.employees} emp &middot; {e.company.industry || ""}</div>
                          </div>
                        </div>
                      ...
                      {e.dm.background && <div style={{ fontSize: 10, color: "#6b7280", marginLeft: 28, marginTop: 4, fontStyle: "italic" }}>{truncate(e.dm.background, 200)}</div>}
                    </div>
                  );
                })}
              </div>
                          ...
                          color: l.src === "Apollo.io" ? "#7c3aed" : l.src === "HubSpot" ? "#f97316" : l.src === "LinkedIn" || l.src === "LinkedIn Jobs" ? "#0077b5" :
                            l.src === "Indeed" ? "#2164f3" : l.src === "ZipRecruiter" ? "#239846" : l.src === "Glassdoor" ? "#0caa41" :
                              l.src === "Google Jobs" ? "#ea4335" : l.src === "Hunter.io" ? "#ff7043" : l.src === "Crunchbase" ? "#0288d1" :
                                l.src === "BLS Data" ? "#1565c0" : l.type === "gate" ? "#d97706" :
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "48px 24px" }}>
      ...
      <div style={{ marginBottom: 8, marginTop: 8 }}>
        ...
        <div style={{ marginLeft: 34, marginBottom: 16 }}>
          <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 10, lineHeight: 1.5 }}>Finds the right decision maker. Verifies their title, tenure, and email. Researches their background for personalized outreach.</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
      ...
      <div style={{ marginBottom: 8, marginTop: 8 }}>
        ...
        <div style={{ marginLeft: 34, marginBottom: 16 }}>
          <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 10, lineHeight: 1.5 }}>Calculates ROI using BLS salary data vs Feather's $0.07/min. Drafts a cold email, LinkedIn connection note, follow-up message, and thought leadership post.</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
